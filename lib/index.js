...
internals.consolidateChunks = function (items, chunks, options) {
    ...
    client.splice(indices.left, (indices.right - indices.left + 1), { range });
...
exports.ranges = function (items, ranges, options) {
    ...
    if ((totalClientItems / totalServerItems) > (options.maxClientStorageRatio || 1.1)) {
        return ['0->'];
    }

    // If the client has reach their limit of chunks, we must consolidate two
    // of their existing chunks first

    if (chunks.length >= (options.maxClientChunks || 5)) {
        chunks = internals.consolidateChunks(items, chunks, options);
    }
