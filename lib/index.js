...
internals.get = function (obj, path) {
    ...
    throw new Error('Doesn\'t support paths with more than 1 .');
};

internals.validItemsPerChunk = function (items, chunks, options) {

    return chunks.map((chunk) => internals.itemsInRange(chunk.from, chunk.to, items, options.path));
};


internals.consolidateChunks = function (items, chunks, options) {

    const validItemsPerChunk = internals.validItemsPerChunk(items, chunks, options);
...
internals.evictEmpty = function (items, chunks, options) {

    const validItemsPerChunk = internals.validItemsPerChunk(items, chunks, options);

    // Remove all chunks which contain zero valid items

    for (let i = 0; i < chunks.length; ++i) {
        if (validItemsPerChunk[i] === 0) {
            chunks.splice(i, 1);
            validItemsPerChunk.splice(i, 1);
            i--;
        }
    }
};
...
exports.ranges = function (items, ranges, options) {
    ...
    // Eagerly evict any empty chunks

    internals.evictEmpty(items, chunks, options);

    // If the client is up to date, return chunks
